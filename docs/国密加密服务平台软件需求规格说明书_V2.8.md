...
| 文档版本 | V2.8                                                                                                                             |
| 编写日期 | 2026-09-24                                                                                                                       |
| 文档状态 | 编码基线候选版（完成 V2.7 审阅收口；MFA 全局强制；手机号去特殊化；平台能力按密评要求设计；业务 API 采用请求直接签名）                                                               |
| 上一版本 | V2.7                                                                                                                             |
| 文档用途 | 需求分析、系统设计、开发实施、测试验收及项目评审                                                                                                         |
| 合规基线 | GB/T 22239-2019（等保2.0三级）、GB/T 39786-2021（密评）、信息技术应用创新（信创）                                                                        |
| 认证基线 | 业务 API：AppID + 请求直接签名（HMAC-SM3，AppSecret 不在网络传输）；管理端：用户名 + 密码 + Token（可访问全部 API）+ MFA 全局强制（MFA_POLICY_MODE，默认 REQUIRED）；密码设备准入认证 |
...
| V2.5 | 2026-09-24 | 业务 API 改为请求直接签名模式；Token 仅限管理端；删除 F-AUTH-020/021/022/024；新增手机号字段；软件密码模块允许生产；删除第 9 章工程结构                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| V2.6 | 2026-09-24 | 采纳评审意见 H0-01～H0-04、H1-01～H1-26、H2-01～H2-05：①AppSecret 改为 SecretCiphertext + SecretHash 存储，解决 HMAC-SM3 验签模型冲突；②Checksum 升级为 IntegrityValue（HMAC-SM3），解决数据库篡改者问题；③拆分平台能力基线与部署合规基线；④补全 A.8 需求编号全清单；⑤四层逻辑密钥体系（L0 Unlock KEK → L1 Root Key → L2 Runtime KEK → L3 Data Key）+ Provider 物理实现解耦（Software / HSM 互斥，剔除混合模式）；⑥Root Key 加载验证改为 KCV/Wrap-Unwrap 方式；⑦DESTROYED 重新定义（在线材料 vs 归档材料）；⑧HSM 密钥备份模型按 Provider 分别定义；⑨请求签名补充跨语言规范（7 项）；⑩Nonce 原子防重放（HMAC 成功后写入）；⑪新增 AdminSession、RefreshToken、KeyMaterial 实体；⑫新增 KeyType 兼容性检查、Permission 权限矩阵；⑬PhoneHash 改为 HMAC-SM3 密钥化；⑭补充密码策略、端到端性能指标；⑮修正 7.14→7.16、7.9→7.11 引用残留（7.16→7.17 的重编由 V2.7 执行）；⑯已删除编号标记 DEPRECATED；⑰D-16 未关闭改为上线前必须确认 |
| V2.7 | 2026-09-24 | 完成 V2.6 定向评审收口：①明确 HSM Master Key 与平台逻辑 Root Key 的物理实现边界；②引入 Provider Capability 模型与可选扩展接口，避免强制所有密码设备实现 Wrap/Backup/Restore/Import/Export；③明确 HSM Root Key 轮换、重包裹、备份恢复均按 Provider 能力执行；④补充 ProviderKeyIdentity/KCV/Fingerprint 恢复校验；⑤补充 GCM Counter 溢出禁止回绕及自动轮换要求；⑥冻结密码 Hash 算法族为 PBKDF2-SM3，并将参数作为详细设计前置基线；⑦将性能需求与部署实例性能基线分离，禁止测试后反向修改需求；⑧修正 7.16→7.17 完整性校验引用；⑨修正历史未来日期；⑩统一审计签名密钥的 Provider-managed Key 语义；⑪明确 ARCHIVE MATERIAL 仅用于历史解密/验签；⑫补充固定签名测试向量与动态时间窗口 E2E 测试要求。                                                                                                                                                                                                     |
| V2.8 | 2026-09-24 | 采纳 V2.7 审阅意见并落实需求方既定决策：①MFA 恢复并全局强制——F-CON-013 改为"全局强制 MFA"，新增 F-CFG-006"MFA 强制策略配置"，明确 MFA_POLICY_MODE（OPTIONAL / REQUIRED，默认 REQUIRED）；②手机号去特殊化——手机号作为用户普通属性字段，统一使用 UserDataDEK 保护，删除 PhoneSearchKey/PhoneHash 机制；③修正 StringToSign 公式歧义——明确 CanonicalHeaders 为各 `key:value\n` 顺序拼接后去除末尾换行，以 3.3.3.3 固定向量为唯一仲裁（H-01）；④GCM Counter 溢出处置收敛至 Data Key/KeyVersion 层，Root Key 不自动触发轮换（M-01），新增平台级调用总量约束 ≤2³²（M-02）；⑤新增 Provider 最小能力准入集（M-03）；⑥第 10 章补齐 Counter 溢出、NotSupported、恢复身份校验测试用例（M-04）；⑦Nonce TTL 调整为不小于时间窗口跨度的 2 倍（默认 6 分钟）（L-03）；⑧恢复被改写的历史记录与"V2.6 新增"标签（A.11/A.8/修订记录 V2.6 条目）（L-01/L-02）。                                                                           |
...
**管理端登录**：采用用户名 + 密码 + 验证码 + MFA（MFA_POLICY_MODE 默认 REQUIRED，全局强制）；登录成功后签发 Access Token，用于访问全部 API。
...
36. 国密 TLS 支持；
37. 管理用户 MFA 全局强制（MFA_POLICY_MODE，默认 REQUIRED）；
38. Root Key/KEK 管理（含 Software / HSM 两种 Provider）；
39. 数据库关键字段完整性校验（IntegrityValue）；
40. 密码模块自检；
41. 用户手机号（加密存储，用于短信告警）。

**说明**：本期建设范围包含管理用户 MFA 能力（V2.8 恢复并全局强制）。系统提供 MFA 绑定、验证、重置能力；强制策略由配置项 MFA_POLICY_MODE（OPTIONAL / REQUIRED）控制，默认 REQUIRED，所有管理用户必须绑定第二因素。
...
| MFA               | Multi-Factor Authentication，多因素认证；V2.8 起管理端全局强制（MFA_POLICY_MODE，默认 REQUIRED） |
...
**最小能力准入集（V2.8，审阅 M-03）**：

可选能力全部放开后，仍须定义 Provider 可被平台使用的最低门槛。平台自身密钥体系（L2 KEK-Runtime Wrap L3 Data Key；Software 模式的 WrappedMaterial 存储）硬依赖 Wrap/Unwrap 能力：

1. **CanWrapKey / CanUnwrapKey 为强制准入项**：Provider 接入（注册/启用）时必须声明并实测通过，否则不得准入；
2. **HSM Provider** 的 Wrap/Unwrap 必须在 HSM 内部完成，密钥明文不得出设备；
3. **SoftwareCryptoProvider（软件密码模块）必须实现全部扩展接口**（Backup/Restore/Import/Export/Rotate/Rewrap 及 Key Identity 校验）；
4. Backup/Restore、Import/Export、RotateRootKey、RewrapKey 为真正可选项：缺失时对应管理功能降级为不可用并在管理控制台明示，不得静默失败；
...
- Counter 溢出属于安全事件/关键运行事件，必须写入审计并触发告警。
...
| 编号         | 功能              | 要求                                                                                     |
| ---------- | --------------- | -------------------------------------------------------------------------------------- |
| F-AUTH-010 | 管理端登录           | 用户名 + 密码 + 验证码 + MFA 第二因素（全局强制，MFA_POLICY_MODE 控制）；登录成功签发 Access Token + Refresh Token |
...
| F-CON-013 | 全局强制 MFA  | 管理端登录全局强制 MFA（V2.8）：通过 MFA_POLICY_MODE（OPTIONAL / REQUIRED）控制，默认 REQUIRED；REQUIRED 模式下所有管理用户必须绑定第二因素，口令校验通过后必须完成 MFA 验证方可获得会话 |
|           |           |                                                                                                                               |
...
管理端登录采用口令认证，并根据风险控制要求使用验证码；MFA 通过配置项 MFA_POLICY_MODE（OPTIONAL / REQUIRED）控制强制策略，默认 REQUIRED（全局强制，V2.8 恢复并全局强制）。

- REQUIRED 模式（默认）：所有管理用户必须绑定 MFA 第二因素；未绑定用户登录后进入强制绑定流程，完成绑定前不授予管理权限；
- MFA 第二因素：TOTP、国密 USBKey、SM2 数字证书、动态令牌等；
- 口令校验通过后必须完成 MFA 第二因素验证，方可获得会话；
- REQUIRED 模式下用户不得停用 MFA；OPTIONAL 模式下用户可自主停用，需二次确认并记录审计；
- MFA 绑定、启用、停用、重置、验证失败均须审计；
- 连续失败锁定、Session 空闲超时（15 分钟）、绝对会话超时（24 小时）、Cookie Secure/HttpOnly/SameSite 等要求保持不变；
- 手机号绑定与变更需发送短信验证码验证。

MFA_POLICY_MODE 默认为 REQUIRED，管理端全局强制启用 MFA（V2.8 决策，原"可选能力"表述废止）。

**等保三级身份鉴别合规论证**：

本平台管理端身份鉴别采用"口令 + 密码技术第二因素（MFA）"组合。针对等保三级"应采用口令、密码技术两种或两种以上组合的鉴别技术，且其中一种鉴别技术至少应使用密码技术来实现"的要求：

1. 口令属于"口令"鉴别技术；
2. MFA 第二因素中的国密 USBKey、SM2 数字证书、动态令牌等基于密码技术实现，构成密码技术鉴别；
3. MFA_POLICY_MODE=REQUIRED（默认）时，全部管理用户均实际执行"口令 + 密码技术"双因素鉴别，直接满足要求；
...
平台配置至少包括：AppSecret 策略、管理端 Token 空闲超时（默认 15 分钟）、管理端 Token 绝对超时（默认 24 小时）、请求签名时间窗口（默认 ±3 分钟）、请求签名 Nonce TTL（默认 6 分钟，不小于时间窗口跨度的 2 倍）、登录失败次数、管理员 Session 超时、MFA 强制策略（MFA_POLICY_MODE，OPTIONAL / REQUIRED，默认 REQUIRED，V2.8）、请求体大小限制、密钥默认有效期、密钥轮换策略、审计保留周期、告警策略、IP 白名单策略（默认关闭）、密码设备策略、国密 TLS 策略、完整性值巡检策略、短信告警配置。
...
| F-CFG-006 | MFA 强制策略配置 | 配置项 MFA_POLICY_MODE（V2.8 新增）：取值 OPTIONAL / REQUIRED，默认 REQUIRED；策略变更需权限校验、高风险二次确认并审计 |
...
| NF-CMP-013 | 系统应支持管理用户 MFA 并支持强制策略配置（V2.8）：通过 MFA_POLICY_MODE（OPTIONAL / REQUIRED）控制，默认 REQUIRED（全局强制）；策略变更需权限校验、二次确认并审计 |
...
**CanonicalHeaders 拼接口径（V2.8 钉死，审阅 H-01）**：与 3.3.3.2 相同——CanonicalHeaders 为各 `key:value\n` 顺序拼接后**去除末尾换行符**的结果，Header 块整体末尾不带换行；以 3.3.3.3 固定测试向量为唯一仲裁依据。
...
算法一致性固定值同 3.3.3.3；`X-Timestamp=1727078400` 仅用于固定算法向量，因已过期，不得作为端到端认证成功用例。端到端测试必须在运行时生成动态 Timestamp，并分别验证有效窗口、窗口边界及过期请求。

固定预期签名：`KfuluxDM90Ze4jQtQeSG5AZ/MubbtzoA3vFoKslQFfc=`。
...
19. **AdminSession**（V2.7 修订，管理端会话）；
20. **RefreshToken**（V2.7 修订）；
...
要求：UserDataDEK 轮换时需重新加密所有手机号。

## 7.5 管理端会话与令牌数据（V2.7 修订）
...
- 密码；
- 验证码/风险控制；
- MFA 全局强制（MFA_POLICY_MODE，默认 REQUIRED）：所有管理用户必须绑定第二因素，登录及高风险操作执行 MFA 验证；
- 高风险操作二次确认：优先使用 MFA，特殊场景（设备故障等）采用密码再确认、验证码等方式并记录审计；
- Token 空闲超时 15 分钟（由 AdminSession.LastAccessAt 实现），绝对超时 24 小时。

MFA_POLICY_MODE 默认为 REQUIRED，管理端全局强制（V2.8）。等保三级身份鉴别合规论证见 3.6.3。
...
SM2/SM3/SM4/HMAC-SM3/随机数密码服务；密码模块自检；密钥全生命周期管理（生成、激活、禁用、轮换、过期、注销、销毁）；密钥版本管理；密钥运算级规则；Root Key/KEK 逻辑分层管理（Software / HSM 两种 Provider）；AppID/AppSecret 管理（加密存储）；**业务 API 请求直接签名认证**；管理端 Access Token 认证 + AdminSession；**资源归属校验**；KeyType 兼容性校验；密钥隔离；审计日志与完整性链 + 外部锚点；管理控制台用户与角色管理；健康检查；密码设备适配（HSM 与软件密码模块）；等保三员分立与互斥；剩余信息保护；**平台密码能力按密评要求设计**；商用密码产品认证核查（HSM 场景）；集中审计外发；数据库关键字段完整性校验（IntegrityValue）；用户手机号加密存储（UserDataDEK）；**管理端 MFA 全局强制（MFA_POLICY_MODE，V2.8）**。

### 9.1.2 P1 本期交付的非核心功能

密钥导入、备份、恢复与恢复验证；密钥轮换策略；安全事件与告警；风险检测；告警规则管理；系统配置管理；数据分区与归档；备份保留周期管理；数据恢复验证；信创兼容性适配；国密 TLS 支持；IP 白名单（可选）；RefreshToken 管理。
...
- **签名输出为 Base64**；
- **固定测试向量跨语言实现一致**。
- **动态 Timestamp E2E**：测试运行时生成当前 Unix Timestamp，验证当前时间、边界 ±3 分钟、超出窗口、Nonce 重放四类场景；固定向量不得作为生产时间窗口成功依据。
...
| 错误码   | 标识符                                | 说明                                                     |
| ----- | ---------------------------------- | ------------------------------------------------------ |
| 10003 | MFA_VERIFY_FAILED                  | MFA 第二因素验证失败；MFA_POLICY_MODE=REQUIRED 时登录必经环节（V2.8 调整） |
...
本节为**全部需求编号的权威索引**，供追踪矩阵（13.1）在详细设计阶段逐项展开。**H0-04 修正：本节为 V2.7 修订，此前 V2.5 缺失**。
...
| F-RK-012      | HSM Root of Trust             | 3.11.6 | HSM Security Domain / HSM Master Key（平台不可直接管理）        | 功能  |
...
V2.7 在 V2.6 已关闭 H0 阻断问题的基础上，针对 HSM 密钥模型、Provider 能力抽象、性能基线、GCM Counter、安全密码参数及全文交叉引用进行定向修订。V2.7 不改变 V2.6 已确认的总体架构与业务认证模式。
